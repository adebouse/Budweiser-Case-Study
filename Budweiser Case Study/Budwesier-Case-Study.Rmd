...
Beers <- read_csv("C:/Users/Antonio Debouse/Desktop/Doing Data Science/GitHub/Budweiser-Case-Study/Beers.csv")
str(Beers)
view(Beers)

Breweries <- read_csv("C:/Users/Antonio Debouse/Desktop/Doing Data Science/GitHub/Budweiser-Case-Study/Breweries.csv")
str(Breweries)
view(Breweries)
...
Answer: There is some missing data in "Style" but I will only address missing data in "ABV" and "IBU" since those are relevant for questions of interest. I will deal with those missing values by using na.rm during aggregation calculations, and complete delete any observations when using k-NN models. 
...
Comment: Based on the barchart visualization, Maine has the highest median IBU followed by West Virginia. However, the District of Columbia and Kentucy appears to be tied for highest ABV median, followed by Maine with the second highest ABV median. 
...
Answer: There appears to be a positive correlation of ABV to IBU (maybe some type of polynomial relationship); higher the ABV appears to result in a higher IBU
...
Comment: The same positive correlation also appears in beers that are only "Ale" and "IPA". The chart shows the visual relationship of IPAs having a higher median ABV of 6.8% and IBU of 70 compared to Ales median ABV of 5.5% and IBU of 30. 
...
```
Our search for the best k by looping for many k and the average of many training / test partition recommends knn = 5 or 6, we will use 5.  

```{r}

##Import Household Median Income Data for 2019
PerState_Data <- read.csv("C:/Users/Antonio Debouse/Desktop/Doing Data Science/GitHub/Budweiser-Case-Study/State Population Data.csv",na=".")
str(PerState_Data)
...
##Arrange data in descending order base on Beer_consump, then population, then median household income, then #of Breweries in State
Count_PerState_Data %>% arrange(desc(Beer_Consump,Population,Med_Household_Inc,`Number of Breweries in State`))%>% print()
